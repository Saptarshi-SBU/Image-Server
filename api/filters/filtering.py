...
def ProcessImageSketchFilter(filepath):
	...
	encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), 70]
	_, img_encoded = cv2.imencode('.jpg', sketch_img, encode_param) # encode converts to bytes
	return base64.b64encode(img_encoded).decode('utf-8')
	#return img_encoded.tostring()

def ProcessImageCartoon(filepath):
	img = cv2.imread(filepath)
	...
	encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), 70]
	_, img_encoded = cv2.imencode('.jpg', cartoon, encode_param) # encode converts to bytes
	return base64.b64encode(img_encoded).decode('utf-8')